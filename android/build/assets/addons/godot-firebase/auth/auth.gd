...
signal send_account_verification_email_completed(message)
...
func _on_FirebaseAuth_request_completed(result : int, response_code : int, headers : PackedStringArray, body : PackedByteArray) -> void:
	...
	if response_code == HTTPClient.RESPONSE_OK:
		...
		elif (res.error && res.error.message == "USER_NOT_FOUND"):
			userdata_deleted.emit()
		else:
...
func send_account_verification_email() -> void:
	if _is_ready():
		...
		if err != OK:
			...
			send_account_verification_email_completed.emit("%s" % err)
		send_account_verification_email_completed.emit("")
